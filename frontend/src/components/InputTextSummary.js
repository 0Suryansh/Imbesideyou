...
const InputTextSummary = () => {
  const [open, setOpen] = React.useState(false);
  const [buttonName, setButtonName] = React.useState("Summarize");
  const [output,setOutput]=React.useState("null")
  const [textInput,setTextInput]=React.useState("")

  React.useEffect(()=>{
    if(textInput===""){
      setButtonName("Summarize")
    }
  },[textInput])

  async function handleOpen() {
    if(textInput===""){
      toast.error('Please Enter some text to Summarize',{...toastStyle.error})
      return
    }
    setButtonName("Summarizing...");
    setOutput("null")
    setOpen(true);
    ...
    if(loggedIn){
      ...
      setOutput(res.data.output_para)
       ...
       await openai.createCompletion({
        ...
        prompt: `Generate a short summary for ${textInput}`,
        temperature: 0.7,
        max_tokens: 256,
        top_p: 1,
        frequency_penalty: 0,
  ...
  return (
    ...
    >
      ...
      >
        Summarize any text with a click of a button. This summarizer can
        condense articles, papers, or documents down to the key points
        instantly.
      </Typography>
      <Typography
        variant="p"
...
export default InputTextSummary;
